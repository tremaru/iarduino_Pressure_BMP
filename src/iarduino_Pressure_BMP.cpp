...
/*	BMP280:	*/	}else if(valID==BMP280_ID){	if(i>4){i=4;}															//	Корректируем значение передискретизации для датчика BMP280
					if(!selI2C->writeByte(valAddress,BMPXXX_ADDRR_CTRL,((i+1)<<5|(i+1)<<2|0b11))){return false;}	//	Eсли не удалось записать байт данных ( OSRS_T[2:0] | OSRS_P[2:0] | MODE[1:0] ) в регистр BMPXXX_ADDRR_CTRL, то возвращаем false.
				}else{return false;}																				//	Если valID <> BMP180_ID и valID <> BMP280_ID значит устройство с адресом valAddress на шине I2C не является датчиком MBP, возвращаем false.
...
//	BMP180:			Читаем «сырые» значения температуры и давления из датчика:										//
					uint8_t j;																						//
					if(selI2C->writeByte(valAddress, BMPXXX_ADDRR_CTRL, 0x2E)){										//	Если в регистр BMPXXX_ADDRR_CTRL записалось значение 0x2E (инициализация расчёта температуры)
						...
						j=50; while( bitRead(selI2C->readByte(valAddress, BMPXXX_ADDRR_CTRL),5) && j){j--;}			//	Ждём установки флага SCO (пятый бит байта) в регистре BMPXXX_ADDRR_CTRL (флаг SCO сигнализирует о завершении расчёта)
					...
					if(selI2C->writeByte(valAddress, BMPXXX_ADDRR_CTRL, (0x34 | valMeasurement<<6) )){				//	Если в регистр BMPXXX_ADDRR_CTRL записалось значение 0x34 или 0x74 или 0xB4 или 0xF4 (инициализация расчёта давления с точностью valMeasurement)
						delay(20);																					//	Даём время датчику на выполнение рассчётов (если опросить датчик пока он занят рассчётами, он создаст помехи на линиях шины I2C и Arduino зависнет).
						j=50; while( bitRead(selI2C->readByte(valAddress, BMPXXX_ADDRR_CTRL),5) && j){j--;}			//	Ждём установки флага SCO (пятый бит байта) в регистре BMPXXX_ADDRR_CTRL (флаг SCO сигнализирует о завершении расчёта)
...
uint32_t	iarduino_Pressure_BMP::readBytes(uint8_t i, uint8_t j, bool f){											//	Объявляем функцию чтения регистров датчика						(аргументы: адрес_регистра, количество_байт, чем_старше_регистр_тем_старше_байт)
				...
				selI2C->readBytes(valAddress, i, data, j);															//	Заполняем массив data читая j байт из модуля valAddress начиная с регистра i
